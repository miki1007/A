...
interface MikixDao {
    ...
    @Query("SELECT strftime('%Y-W%W', datetime(s.completedAt/1000, 'unixepoch')) AS weekStart, SUM(s.weight*s.reps) AS volume FROM SetEntry s GROUP BY weekStart ORDER BY weekStart")
    ...
    @Query("SELECT * FROM GroupEntity ORDER BY memberCount DESC")
    fun groups(): Flow<List<GroupEntity>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertFeed(posts: List<FeedPostEntity>)

    @Query("SELECT * FROM FeedPostEntity ORDER BY createdAt DESC")
    fun feed(): Flow<List<FeedPostEntity>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertChallenges(challenges: List<ChallengeEntity>)

    @Query("SELECT * FROM ChallengeEntity ORDER BY progress DESC")
